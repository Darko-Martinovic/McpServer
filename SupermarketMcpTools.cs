...
public static class SupermarketMcpTools
{
    ...
    public static async Task<string> GetSalesData(ISupermarketDataService dataService, [Description("Start date in YYYY-MM-DD format")] string startDate, [Description("End date in YYYY-MM-DD format")] string endDate)
    {
        try
        {
            Serilog.Log.Information("MCP Tool 'GetSalesData' called with startDate: {StartDate}, endDate: {EndDate}", startDate, endDate);

            if (!DateTime.TryParse(startDate, out var start) || !DateTime.TryParse(endDate, out var end))
            {
                Serilog.Log.Warning("MCP Tool 'GetSalesData' validation failed: Invalid date format");
                return "Invalid date format. Please use YYYY-MM-DD format.";
            }

            var salesData = await dataService.GetSalesDataAsync(start, end);
            var result = JsonSerializer.Serialize(salesData, new JsonSerializerOptions { WriteIndented = true });
            Serilog.Log.Information("MCP Tool 'GetSalesData' completed successfully. Returned {Count} records", salesData.Count());
            return result;
        }
        catch (Exception ex)
        {
    ...
    public static async Task<string> GetTotalRevenue(ISupermarketDataService dataService, [Description("Start date in YYYY-MM-DD format")] string startDate, [Description("End date in YYYY-MM-DD format")] string endDate)
    {
        try
        {
            Serilog.Log.Information("MCP Tool 'GetTotalRevenue' called with startDate: {StartDate}, endDate: {EndDate}", startDate, endDate);

            if (!DateTime.TryParse(startDate, out var start) || !DateTime.TryParse(endDate, out var end))
            {
                Serilog.Log.Warning("MCP Tool 'GetTotalRevenue' validation failed: Invalid date format");
                return "Invalid date format. Please use YYYY-MM-DD format.";
            }
            ...
            Serilog.Log.Information("MCP Tool 'GetTotalRevenue' completed successfully. Revenue: ${Revenue:N2}", revenue);
            return result;
        }
        catch (Exception ex)
        {
    ...
    public static async Task<string> GetLowStockProducts(ISupermarketDataService dataService, [Description("Stock threshold level")] int threshold = 10)
    {
        try
        {
            Serilog.Log.Information("MCP Tool 'GetLowStockProducts' called with threshold: {Threshold}", threshold);

            var lowStockProducts = await dataService.GetLowStockProductsAsync(threshold);
            var result = JsonSerializer.Serialize(lowStockProducts, new JsonSerializerOptions { WriteIndented = true });
            Serilog.Log.Information("MCP Tool 'GetLowStockProducts' completed successfully. Returned {Count} products", lowStockProducts.Count());
            return result;
        }
        catch (Exception ex)
        {
    ...
    public static async Task<string> GetSalesByCategory(ISupermarketDataService dataService, [Description("Start date in YYYY-MM-DD format")] string startDate, [Description("End date in YYYY-MM-DD format")] string endDate)
    {
        try
        {
            Serilog.Log.Information("MCP Tool 'GetSalesByCategory' called with startDate: {StartDate}, endDate: {EndDate}", startDate, endDate);

            if (!DateTime.TryParse(startDate, out var start) || !DateTime.TryParse(endDate, out var end))
            {
                Serilog.Log.Warning("MCP Tool 'GetSalesByCategory' validation failed: Invalid date format");
                return "Invalid date format. Please use YYYY-MM-DD format.";
            }

            var categorySales = await dataService.GetSalesByCategoryAsync(start, end);
            var result = JsonSerializer.Serialize(categorySales, new JsonSerializerOptions { WriteIndented = true });
            Serilog.Log.Information("MCP Tool 'GetSalesByCategory' completed successfully. Returned {Count} categories", categorySales.Count());
            return result;
        }
        catch (Exception ex)
        {
